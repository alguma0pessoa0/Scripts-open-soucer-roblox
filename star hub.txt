--DEOBFUSCATED BY SOURCES HUB DEOBFUSCATOR

-- OR's Duel Hub v4 - FINAL [AutoSteal build]
-- Funcoes integradas do source_code_by_orzk

-- ============================================================
-- RESOLVE fireproximityprompt (compativel com key system / loadstring)
-- Quando o script roda encapsulado, a funcao some do ambiente local
-- mas ainda existe no getgenv() do executor
-- ============================================================
if not fireproximityprompt then
    fireproximityprompt = (getgenv and getgenv().fireproximityprompt)
        or (genv and genv().fireproximityprompt)
        or function(prompt)
            -- Fallback: tenta chamar via metodo do proprio prompt
            pcall(function()
...
-- Todos os dados persistidos ficam em ORsDuel_Config.json
-- ============================================================
...
-- Config padrao embutido: aplicado na PRIMEIRA execucao (quando nao existe arquivo salvo).
-- Garante que todo novo usuario comeca com as posicoes e toggles definidos pelo criador.
...
local function _loadGlobalConfig()
    ...
    -- Primeira execucao: aplica os defaults do criador e salva o arquivo
    if not loaded then
...
-- Debounce: agrupa multiplas chamadas rapidas em uma unica escrita ao disco
-- evita corrupcao de arquivo quando varios hubs salvam ao mesmo tempo
...
-- cfgGet: le uma secao do config unificado (retorna nil se nao existe)
local function cfgGet(section)
    return _globalConfig[section]
end

-- cfgSet: atualiza memoria + agenda escrita debounced
local function cfgSet(section, data)
    _globalConfig[section] = data
    _saveDebounced()
end

-- cfgSetNow: igual ao cfgSet mas escreve imediatamente (usado em saves criticos)
...
-- Backup periodico: salva a cada 8s para nao perder posicoes se o jogo fechar durante drag
task.spawn(function()
    while true do
        task.wait(8)
        _saveGlobalConfig()
...
-- ============================================================
-- INTRO: "Orksz Duels > Nine Duels" centralizado, sem fundo
-- ============================================================
task.spawn(function()
    ...
    -- Um Ãºnico label centralizado na tela
    ...
    -- Entra vindo de cima (Y comeÃ§a em 0.3, vai pra 0.45)
    ...
    -- Digita tudo num Ãºnico label: "Orksz Duels > Nine Duels"
    -- "Orksz Duels" em branco, " > Nine Duels" em cinza via cor Ãºnica (sem RichText)
    local part1 = "⭐Star Hub"
    local part2 = " On Top⭐"
    ...
    -- Sai deslizando pra direita + fade
...
local C = {
    ...
    Gray      = Color3.fromRGB(136, 0, 255),
    LightGray = Color3.fromRGB(136, 0, 255),
    White     = Color3.fromRGB(255, 255, 255),
    Dim       = Color3.fromRGB(90,  90,  90),
    TogOff    = Color3.fromRGB(40,  40,  48),
}

local STROKE_ACTIVE = Color3.fromRGB(176, 0, 250)  -- vermelho escuro: aparece ao ativar/clicar
local STROKE_IDLE   = Color3.fromRGB(0, 0, 0)      -- invisivel: sem borda em repouso
...
local function addDefaultStroke(frame)
    ...
    stroke.Transparency = 1  -- invisivel em repouso
    stroke.Enabled = true
    return stroke
end

local _hubRestoring = false -- quando true, hubOpen nao anima
local tFrameGlobal = nil   -- taunt frame exposto para intro

-- Chama logo apos setar Size de qualquer hub/sub-hub
...
-- Sub-hubs: sem animacao, sÃ³ show/hide simples
...
-- Aplica o mesmo gradiente preto do Hub Principal em qualquer frame
-- Tabela global de gradientes animados
local function applyHubGradient(frame)
    -- Fundo preto solido igual ao hub principal
...
local function setHubActive(hub, dot, active)
    ...
    if active then
        ...
        stroke.Transparency = 0  -- mostra borda vermelha escura
    else
        stroke.Color = STROKE_IDLE
        stroke.Transparency = 1  -- esconde borda
    end
    if dot then
        dot.Text = "🟣"
...
-- Sincroniza settings frame abaixo do hub principal em tempo real (move junto)
...
-- Cria um toggle de expansao inline para um hub:
-- ao clicar na engrenagem, o hub cresce/encolhe animado para revelar settings embutidos.
-- collapsedH = altura fechado (ex: 34), expandedH = altura aberto (ex: 108)
local function makeInlineExpand(frame, collapsedH, expandedH)
    ...
    return function()
        ...
        -- Quando expande: sobe ZIndex pra ficar na frente de tudo
        -- Quando fecha: volta ao ZIndex original
...
-- Flag global: true por um breve momento apos qualquer drag de sub-hub
-- Usado pelos botoes para nao ativar acidentalmente apos arrastar
local _dragJustHappened = false  -- flag global legada (compatibilidade)

-- Click absorber: apos drag no mobile, coloca overlay invisivel que consome
-- o proximo toque falso que o Roblox dispara automaticamente no TouchEnded
local function _absorbNextClick()
    ...
    -- Consome o proximo click e se destroi
    local conn
    ...
    -- SeguranÃ§a: destroi apÃ³s 600ms caso nao venha clique falso
...
-- ============================================================
-- MAKEDRAG v3 - mobile-safe com absorÃ§Ã£o ativa de clique falso
-- ============================================================
local function MakeDrag(frame, onDrop)
    ...
    -- Threshold: 12px no touch, 4px no mouse
    ...
    local function hookObj(obj)
        if obj:IsA("ScrollingFrame") then return end  -- nao hookeia scroll
        if obj:IsA("GuiObject") and not obj:IsA("TextBox") then
            -- Nao hookeia descendentes de ScrollingFrame
            local p = obj.Parent
            while p do
    ...
    UIS.InputChanged:Connect(function(input)
        ...
        if not didMove then
            ...
            didMove = true
            -- IMPORTANTE: seta o flag AQUI, antes do InputEnded disparar.
            -- No Roblox, MouseButton1Click do elemento filho dispara ANTES do
            -- UIS.InputEnded global, entao o flag precisa estar ativo ja neste ponto.
            _dragJustHappened = true
        end
    ...
    UIS.InputEnded:Connect(function(input)
        ...
        if didMove then
            ...
            if wasMobile then
                -- Mobile: absorve o clique fantasma que o Roblox dispara apos drag
                -- _absorbNextClick ja reseta _dragJustHappened apos consumir o clique
                _absorbNextClick()
            else
                -- Mouse: reseta o flag apos um breve delay (suficiente para o
                -- MouseButton1Click do botao ja ter sido bloqueado pelo guard)
...
do
    ...
    tpDownStroke.Transparency = 1  -- sem borda em repouso
    ...
    tpDownBtn.MouseButton1Click:Connect(function()
        if _dragJustHappened then return end
        -- Flash de borda vermelha ao clicar
...
mStroke.Transparency = 1  -- sem borda em repouso
...
hStroke.Transparency = 1  -- sem borda em repouso
-- Fundo preto solido igual ao logo (sem gradiente animado)

-- Drag do MainFrame com save de posicao
do
    ...
    MainFrame.InputBegan:Connect(function(i)
        ...
        -- SÃ³ inicia drag se o toque for na area do header (acima do ContentArea)
        local relY = i.Position.Y - MainFrame.AbsolutePosition.Y
        if relY > HEADER_H then return end  -- toque no conteudo = nao arrasta o frame
...
-- Titulo ORKSZ DUELS com animacao preto/branco
local hubTitleLbl = New("TextLabel", {
    Text = "STAR DUELS", TextColor3 = Color3.fromRGB(255, 255, 255),
...
-- ============================================================
-- AUTO STEAL state (sistema baseado em animalCache - EGO style)
...
local stealActive_    = false  -- estado interno (controlado pelo toggle)
local animalCache     = {}     -- lista de {name,plot,slot,worldPosition,uid}
local promptCache     = {}     -- uid -> ProximityPrompt
local stealCache      = {}     -- prompt -> {holdCallbacks,triggerCallbacks,ready}
...
local gbStroke       = nil
local radBox         = nil   -- exposto para Settings
local durBox         = nil   -- exposto para Settings

-- Save/Load posicao das barras (config unificado, separadas)
...
-- â”€â”€ BARRA FPS/PING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
topStroke.Transparency = 1  -- sem borda em repouso
do
    local title = Instance.new("TextLabel", topFrame)
    title.Size = UDim2.new(1, 0, 1, 0)
    title.Position = UDim2.new(0, 0, 0, 0)
    title.BackgroundTransparency = 1
    title.Text = "STAR DUEL"
    ...
    -- Atualiza FPS e Ping com media movel para evitar flickering
...
-- â”€â”€ BARRA GRAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
botStroke.Transparency = 1  -- sem borda em repouso
gbStroke = botStroke
do
    -- READY label (esquerda)
...
-- InfoBar: posicao fixa com animacao de bob (nao arrastavel)
do
...
local function criarSpdBillboard(char)
    ...
    -- BillboardGui: so o numero, sem fundo nem caixa
    ...
    -- Label direto na billboard, sem frame nem fundo
    ...
    -- Gradiente preto->branco no proprio texto
    ...
    -- Stroke preto para legibilidade
...
-- Loop de atualizaÃ§Ã£o em tempo real (RenderStepped = todo frame)
...
end -- end do: SPEED ACIMA DA CABECA
...
-- ============================================================
-- SPEED BYPASS - hookeia debug.info pra esconder source dos loops de speed
-- ============================================================
do
    ...
    if ok and dbg and dbg.info and hookfunction then
        ...
        oldInfo = hookfunction(dbg.info, function(...)
            local s = oldInfo(1, "s")
            -- Esconde qualquer checagem de source que envolva speed/velocity
...
-- ============================================================
-- ============================================================
-- KEYBINDS GLOBAIS (centralizadas no Settings)
-- ============================================================
...
-- Labels de display (atualizados pelo Settings ao receber tecla)
...
-- ============================================================
-- BRIDGES GLOBAIS: estado compartilhado entre DuelHub e SpeedHub
-- ============================================================
local mvSpeedOn    = false  -- true quando Movement Speed estiver ativo
local mvSpeedVal   = 40     -- valor do Move Speed (bridge global)
local _bridgeDisableMv = nil  -- para mvSpeed + atualiza visual, preenchida pelo createSpeedHub
local _bridgeSetMvSpeed = nil -- seta mvSpeed local do SpeedHub de fora
local autoPlayOn = false   -- true quando Auto Play estiver rodando
local pararAutoPlayGlobal = nil  -- exposta para AIM/AIM BAT desativarem o autoPlay

-- ============================================================
-- SPEED WHILE STEALING (usado pelo AutoDuel internamente via SpeedHub)
-- ============================================================
local duelReturnSpeed  = 28.8
local duelNotStealSpeed = 52    -- speed base do AutoDuel (ida ao slot), atualizado pelo SpeedHub

-- ============================================================
-- BRAINROT SPEED WATCHER
-- Detecta Stealing no player (mesmo metodo do script externo)
-- ============================================================
do
    ...
    lp:GetAttributeChangedSignal("Stealing"):Connect(function()
        ...
        if stealing and not lastStealing then
            lastStealing = true
            -- Muda pro AFTER STEAL se qualquer speed system estiver ativo
    ...
    -- (heartbeat removido - GetAttributeChangedSignal ja cobre tudo)
end
...
-- ============================================================
-- MELEE AIMBOT + NO ANIM (isolados em do..end);
-- ============================================================
local stopCebo
...
-- ============================================================
-- OPTIMIZER / XRAY / ESP  (isolados em do..end para liberar locais)
-- ============================================================
...
-- ============================================================
-- SPIN HUB - createSpinHub do original
-- ============================================================
local spinGui = nil
...
local function createSpinHub()
    ...
    local function carregarDados()
        ...
        if d then
            -- Migra formato antigo (Scale) para pixels
    ...
    -- Divisor e settings inline (revelados quando hub expande)
...
-- ============================================================
-- AUTO DUEL HUB - createDuelHub do original (completo)
-- ============================================================
local duelGui    = nil
dbFrame          = nil  -- drop brainrot frame (global)
-- ============================================================
-- AUTO STEAL (sistema K7 integrado) - usa a Grab Bar
-- ============================================================
-- â”€â”€ STEAL: funÃ§Ãµes base (EGO style) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
local function findPromptCached(ad)
    ...
    -- Tenta PromptAttachment primeiro, depois busca recursiva
...
-- Retorna o animalData mais prÃ³ximo dentro do raio (para steal)
...
-- Retorna a posiÃ§Ã£o do alvo mais prÃ³ximo SEM limite (para look-at)
local function findNearestTarget()
    local _, bestD = nearestAnimal()  -- pega distÃ¢ncia do mais prÃ³ximo
    ...
    -- Retorna a BasePart do spawn se possÃ­vel, senÃ£o usa worldPosition
    ...
    -- Fallback: cria um Vector3 wrapper virtual
    return { Position = best.worldPosition }
end

-- Scan inicial + watch de novos plots
task.spawn(function()
    task.wait(2)
    ...
    -- Refresh a cada 5s (novos animais)
    task.spawn(function()
...
local function resetStealBar()
    ...
    if gbStroke then
        ...
        gbStroke.Transparency = 1  -- esconde borda ao resetar
    end
end

-- Acha a BasePart mais proxima na hierarquia de um objeto
...
local autoTPHubFrame   = nil  -- referencia ao frame do hub Auto TP (setada dentro de createDuelHub)
local hitTPHubFrame    = nil  -- referencia ao frame do hub Hit TP (setada dentro de createDuelHub)
local meleeAimHubFrame = nil  -- referencia ao frame do hub Melee Aimbot
...
local function createDuelHub()
    ...
    -- Posicoes: carrega do config se salvo, senao usa as do usuario
    ...
    local autoPlayWasActive = false  -- persiste durante morte/respawn
    local ladoEscolhido = nil  -- SEMPRE comeÃ§a nil (bloqueado), nunca salva lado

    -- â”€â”€ FRAME PRINCIPAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    autoStroke.Transparency = 1  -- sem borda em repouso
    ...
    -- Engrenagem igual ao AIM BAT: fundo transparente, stroke cinza, icone cinza
    ...
    -- â”€â”€ PAINEL (filho do duelGui, posicao sincronizada com fAuto) â”€
    -- Conteudo do painel inline dentro do fAuto (Y >= 34)
    -- O fAuto vai de 34 para 122px quando a engrenagem for clicada

    -- Divisor igual aos outros hubs
    ...
    -- Botao RIGHT (esquerda do painel) - texto branco
    ...
    sPR.Thickness = 1.5; sPR.Color = STROKE_ACTIVE  -- vermelho escuro ao selecionar
    sPR.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    sPR.Enabled = false

    -- Botao LEFT (direita do painel) - texto branco
    ...
    sPL.Thickness = 1.5; sPL.Color = STROKE_ACTIVE  -- vermelho escuro ao selecionar
    sPL.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    sPL.Enabled = false

    -- Bolinha indicadora centralizada abaixo dos botoes
    ...
    -- Keybind: gerenciada pelo Settings (upvalues globais)
    -- autoKeyBind / autoWaitingKey declarados no topo do script
    -- Restaura silenciosamente (display fica no Settings)
    ...
    -- (keybind do Auto Play gerenciada pelo Settings)
    ...
    -- Drag: salva posicao (painel ja segue via Heartbeat)
        ...
        heartbeatL = RunService.Heartbeat:Connect(function()
            ...
            if dist < 1.5 then
                ...
                elseif phase == 2 then
                    ...
                    -- Chegou no R_EXTRA: ativa AFTER STEAL
        ...
        heartbeatR = RunService.Heartbeat:Connect(function()
            ...
            if dist < 1.5 then
                ...
                elseif phase == 2 then
                    ...
                    -- Chegou no P_EXTRA: ativa AFTER STEAL
                    if mvSpeedOn and _bridgeSetMvSpeed then _bridgeSetMvSpeed(duelReturnSpeed) end
    ...
    -- ============================================================
    -- ANTI-LAGBACK: olha para o Brainrot mais proximo (podium alvo)
    -- Loop completamente independente - so observa AutoPlayAtivo
    -- ============================================================
    do
        local lookAlign  = nil
        ...
        RunService.Heartbeat:Connect(function(dt)
            ...
            if active then
                ...
                local hrp  = char and char:FindFirstChild("HumanoidRootPart")
                if hrp then
                    -- Recria align se sumiu (morte/respawn)
                    if not lookAlign or not lookAlign.Parent then
                        setupLook(hrp)
                    end

                    -- Olha para o Brainrot mais proximo (sem limite de distÃ¢ncia)
                    ...
                    if targetPos then
                        ...
                        -- Sem alvo: desliga o align (deixa o personagem livre)
                        if lookAlign then lookAlign.Enabled = false end
                    end

                    -- Micro zigzag anti-lagback ainda ativo
                    local vel = hrp.AssemblyLinearVelocity
    ...
    local function iniciarAutoPlay()
        ...
        -- Se ja tiver Brainrot na mao ao iniciar, comeÃ§a com AFTER STEAL
    ...
    local function hasBrainrotInHand()
        -- Checa atributo Stealing do jogo (mais confiavel)
    ...
    local function pararAutoPlay()
        ...
        if heartbeatL then heartbeatL:Disconnect(); heartbeatL = nil end
        if heartbeatR then heartbeatR:Disconnect(); heartbeatR = nil end
        -- SÃ³ restaura NOT STEALING se nao estiver roubando
        if mvSpeedOn and _bridgeSetMvSpeed and not lp:GetAttribute("Stealing") then _bridgeSetMvSpeed(duelNotStealSpeed) end
        atualizarBtnAutoVisual()
        local char = lp.Character
    ...
    btnAuto.MouseButton1Click:Connect(function()
        if _dragJustHappened then return end
        if not ladoEscolhido then return end
        -- Funciona se SPEED ou MOVEMENT SPEED estiver ativo
    ...
    -- Keybind global para AUTO PLAY (PC)
    ...
    -- Auto-reinicia ao renascer usando o mesmo ladoEscolhido
    lp.CharacterAdded:Connect(function(char)
    ...
    local function pararDuelCompleto()
        ...
        local char = lp.Character
        if char then local hum = char:FindFirstChildOfClass("Humanoid"); if hum then hum:Move(Vector3.zero) end end
        -- NAO chama forceDuelToggle aqui - evita loop de callbacks
    end
    stopDuelExterno = pararDuelCompleto

    -- ============================================================
    -- TP MANUAL FRAME (Auto TP Hub) - criado dentro do createDuelHub
    -- ============================================================
    ...
    autoTPStroke.Transparency = 1  -- sem borda em repouso
    ...
    -- Atualiza visual do botao conforme lado escolhido (bloqueado ou livre)
    ...
    -- Sempre que o lado mudar (via btnPR/btnPL), atualiza o visual do botao TP
    ...
    -- Logica principal do clique
    -- ============================================================
    -- FUNCAO TP REUTILIZAVEL (usada pelo AUTO TP e pelo HIT TP)
    -- ============================================================
    local function executarTP()
        ...
        if hrp then
            ...
            -- Aplica AFTER STEAL speed imediatamente no teleporte
    ...
    -- Keybind Auto TP (gerenciada pelo Settings)
    UIS.InputBegan:Connect(function(input, gp)
    ...
    -- ============================================================
    -- HIT TP - teleporta automaticamente ao perder o Brainrot
    -- ============================================================
    do
    ...
    storeHubSize(fHitTP)
        ...
        hitTPStroke.Transparency = 1  -- sem borda em repouso
        ...
        -- Loop de deteccao: usa StateChanged do Humanoid para pegar o ragdoll
        local stateConn  = nil
        local charConn   = nil
        ...
        local function conectarEstado(char)
            ...
            stateConn = hum.StateChanged:Connect(function(_, newState)
                ...
                -- Dispara quando entrar em ragdoll/physics (levou hit)
        ...
        -- Keybind Hit TP (gerenciada pelo Settings)
        UIS.InputBegan:Connect(function(input, gp)
        ...
        -- Expoe para o toggle do Hub controlar visibilidade (igual autoTPHubFrame)
        fHitTP.Visible = false
        hitTPHubFrame = fHitTP
    end

    -- Sincroniza dot quando o auto play para por outros meios (ex: chegou no fim)
    RunService.Heartbeat:Connect(function()
    ...
    -- Expoe para o toggle do Hub controlar visibilidade
...
-- ============================================================
-- MINI HUB DE POSIÃ‡Ã•ES - compacto, centro da tela
-- ============================================================
(function()
    ...
    posGui.Enabled = false  -- sÃ³ aparece quando o toggle ativa
    ...
    for i, key in ipairs(KEYS) do
        ...
        -- BotÃ£o SET (seta posiÃ§Ã£o atual)
    ...
    -- Expor globalmente para o menu principal abrir
    getgenv().__posHubGui = posGui
end)();


-- ============================================================
-- LOCK FRAME / AIMBOT - do original (100%)
-- ============================================================
local AimSpeed = 55
...
startAimBot = function()
    ...
    aimConnections.aimbot = RunService.Heartbeat:Connect(function()
        ...
        -- Recria align se necessario (respawn)
        ...
        -- Usa sempre o HRP (target) como posicao real - o Torso cai no chao durante ragdoll
        local aimPos  = target.Position
        local diff    = aimPos - h.Position
        local flatDir = Vector3.new(diff.X, 0, diff.Z)

        -- Gira corpo via AlignOrientation
        ...
        local diffY    = diff.Y
        -- Gruda no Y do alvo como fita, sem threshold
...
-- ============================================================
-- TAUNT BUTTON - independente, sempre visivel
-- ============================================================
(function()
    ...
    do
        ...
        if d then
            -- Migra formato antigo (Scale pos.X/Y) para pixels
            if d.pos then
    ...
    -- Posicao: usa pixel salvo ou fallback Scale para o canto inferior direito
    ...
    local tStroke = addDefaultStroke(tFrame)  -- invisivel em repouso
    ...
    tBtn.MouseButton1Click:Connect(function()
        ...
        local msg = "👑⭐Star Hub on top⭐👑"
        -- Metodo principal: SayMessageRequest (chat publico padrao do Roblox)
        -- TextChatService (sistema novo - mais rapido, sem WaitForChild)
        local sent = false
        pcall(function()
        ...
        -- SayMessageRequest (sistema legado)
...
local FloatFrame   = nil  -- forward, definido abaixo
local floatRedDot  = nil  -- forward
local _floatPowerRef = nil  -- referencia para tabela de dados do float (setada no do-block da GUI)

local function startFloat()
    -- Desconecta conexÃ£o anterior se existir (evita travar)
    ...
    floatHubConn = RunService.Heartbeat:Connect(function()
        ...
        -- Exclui TODOS os personagens de players para evitar falso hit
        ...
        -- Valida: se hit.Position.Y > root.Position.Y o cast bateu em algo acima (impossivel, ignora)
        ...
        if math.abs(yDiff) > 0.25 then
            local yVel = yDiff > 0
                and math.clamp(yDiff * 12, 0, 40)    -- sobe
                or  math.clamp(yDiff * 5, -10, 0)    -- desce
...
-- Divisor inline (gear abre frame que mostra sÃ³ keybind â€” gerenciada pelo Settings)
...
local aimValueBox = Instance.new("TextBox", LockFrame) -- mantido pois Ã© upvalue usada em Settings
aimValueBox.Visible = false; aimValueBox.Size = UDim2.new(0,0,0,0)
aimValueBox.Parent = LockFrame

-- (keybind do Aim gerenciada pelo Settings)
UIS.InputBegan:Connect(function(input, gp)
    if gp then return end
...
-- Toggle expansao inline: LockFrame cresce de 34 para 110px
...
local function execSteal(prompt, animalName)
    ...
    if gbStroke then
        ...
        gbStroke.Transparency = 0  -- mostra borda vermelha ao roubar
    end
    ...
    task.spawn(function()
        -- Dispara holdCallbacks (inÃ­cio do hold)
        for _, fn in ipairs(data.holdCallbacks) do task.spawn(fn) end
        -- Espera a duraÃ§Ã£o configurÃ¡vel
        local elapsed = 0
        while elapsed < STEAL_DURATION do elapsed = elapsed + task.wait() end
        -- Dispara triggerCallbacks (trigger = roubo efetivo)
...
-- ============================================================
-- MELEE AIMBOT HUB - sub-hub sem engrenagem
-- ============================================================
do
    ...
    local maActive = false  -- estado interno isolado
    ...
    -- Reconecta ao respawn se estiver ativo
    lp.CharacterAdded:Connect(function()
        task.wait(1)
...
-- ============================================================
-- ============================================================
-- AIM - mesmo sistema do AIM BAT + bat automÃ¡tico contÃ­nuo
-- ============================================================
local autoBatHubFrame = nil
abEnabled  = false   -- global: acessivel de fora
abFrame    = nil
abRedDot   = nil
stopAbAim  = nil
startAbAim = nil
do
    ...
    startAbAim = function()
        ...
        -- Equipa bat movendo diretamente pro character
        local c0 = lp.Character
        if c0 then
            local hum0 = c0:FindFirstChildOfClass("Humanoid")
            if hum0 then
        ...
        abConnections.main = RunService.Heartbeat:Connect(function()
            ...
            -- Recria align se necessÃ¡rio (respawn)
            ...
            -- Bat contÃ­nuo: usa ferramenta equipada no momento (permite trocar)
    ...
    -- Frame â€” idÃªntico ao LockFrame
    ...
    abGear.Text = "⚙️"; abGear.BackgroundTransparency = 1
    ...
    -- (keybind do Aim Bat gerenciada pelo Settings)
    UIS.InputBegan:Connect(function(input, gp)
        if gp then return end
...
(function()
    ...
    _floatPowerRef = floatDadosSalvos  -- permite que startFloat leia power dinamicamente

    -- Frame principal (130x46 - identico ao LockFrame)
    ...
    -- Dot indicador (igual ao aimRedDot)
    ...
    -- Botao principal "FLOAT" (igual ao lockBtn)
    ...
    -- Divisor + settings inline para Float (revelados quando FloatFrame expande)
    ...
    -- Hint power (valor editÃ¡vel no Settings)
    ...
    -- MantÃ©m a referencia de power vÃ¡lida via _floatPowerRef (jÃ¡ setado antes)
    UIS.InputBegan:Connect(function(input, gp)
        if gp then return end
    ...
    -- Botao principal toggle float
    floatMainBtn.MouseButton1Click:Connect(function()
        if _dragJustHappened then return end
        floatEnabled = not floatEnabled
        setHubActive(FloatFrame, floatRedDot, floatEnabled)
        if floatEnabled then
            -- So nao inicia fisica se AutoFloat estiver explicitamente ON
            if not (_floatPowerRef and _floatPowerRef.autoFloatEnabled == true) then
                startFloat()
            end
        else
...
-- ============================================================
-- AUTO FLOAT - fora do do block para evitar limite de locals
-- ============================================================
...
do
    ...
    -- Garante que o campo existe no _floatPowerRef antes de qualquer check
    if _floatPowerRef then
        _floatPowerRef.autoFloatEnabled = false
    end

    -- UI - busca o FloatSettings frame
    ...
    if floatSF then
        ...
        local function doAutoFloat()
            ...
            local afActivated = false  -- true quando FOI o auto float que ligou o floatEnabled

            -- Desliga o float (fisico + visual) ao completar um roubo
            ...
            autoFloatConn = RunService.Heartbeat:Connect(function()
                ...
                -- Nao ativa auto float se estiver roubando
                ...
                -- Nao ativa auto float se estiver com Brainrot na mao
                ...
                if close then
                    -- Inimigo chegou perto: inicia a fisica do float se nao estiver rodando
                    if not floatHubConn then
                        if not floatEnabled then
                            -- Float estava desligado: auto float liga ele
                            afActivated = true
                            floatEnabled = true
                    ...
                    -- Inimigo foi embora: para a fisica do float
                    if floatHubConn then
                        stopFloat()
                        if afActivated then
                            -- Auto float tinha ligado: desliga o float completamente
                            afActivated = false
                            floatEnabled = false
                            setHubActive(FloatFrame, floatRedDot, false)
                        end
                        -- Se float foi ligado manualmente (afActivated=false):
                        -- mantÃ©m floatEnabled=true (dot aceso), mas fisica parada.
                        -- Quando inimigo chegar de novo, startFloat() sera chamado.
                    else
                        afActivated = false
                    end
                end
        ...
        afTogBtn.MouseButton1Click:Connect(function()
            ...
            _floatPowerRef.autoFloatEnabled = autoFloatEnabled  -- sincroniza estado
            setAFVisual(autoFloatEnabled)
            if autoFloatEnabled then
                -- Ligou Auto Float: se o float ja estava ativo, para a fisica agora
                -- (ela so volta quando inimigo chegar)
                if floatEnabled and floatHubConn then
                    stopFloat()
                end
                doAutoFloat()
            else
                -- Desligou Auto Float: para o loop
                if autoFloatConn then autoFloatConn:Disconnect(); autoFloatConn = nil end
                _floatPowerRef.onGrabComplete = nil  -- remove o hook de roubo
                -- Se float ainda esta ativo (manualmente), reinicia a fisica normalmente
                if floatEnabled then
                    startFloat()
                end
            end
...
-- ============================================================
-- SPEED HUB (Speed Customizer) - SPIN do original, sem UNWALK
-- ============================================================
...
local function createSpeedHub()
    ...
    local function carregarSpeed()
        ...
        -- Restaura o estado ativo do speed
        speedWhileStealActive = speedDados.speedActive == true
    end
    carregarSpeed()
    duelReturnSpeed   = speedDados.rtnSpeed     or 28.8
    duelNotStealSpeed = speedDados.notStealSpeed or 52   -- sincroniza variavel de modulo
    -- notStealSpeed sera aplicado ao SpeedValueLeft/Right via referencia externa
    ...
    -- Aplica estado visual inicial conforme o que foi salvo
    ...
    -- â”€â”€ DIVISOR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    -- â”€â”€ MOVEMENT SPEED â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    mvSpeedOn = mvOn  -- sincroniza bridge global
    local mvSpeed      = speedDados.mvSpeed      or 40
    mvSpeedVal = mvSpeed  -- sync global
    local mvConn       = nil

    -- Bridge para atualizar mvSpeed local de fora do closure
    _bridgeSetMvSpeed = function(v)
        mvSpeed = v; mvSpeedVal = v
    end

    local mvStealing    = false  -- espelha o atributo Stealing do player
    local mvStealConn   = nil    -- listener do atributo
    ...
    local function stopMvSpeed()
        ...
        -- Zera velocidade horizontal ao parar
        local char = lp.Character
        local hrp  = char and char:FindFirstChild("HumanoidRootPart")
        if hrp then
    ...
    -- Inicia o loop sem resetar flags (usado internamente)
    ...
    -- Registrar bridges globais para uso externo (ex: iniciarAutoPlay/pararAutoPlay)
    ...
    -- Toggle ENABLE â€” alinhado igual ao SPEED de cima
    ...
    -- Bridge: para mvSpeed E atualiza visual (registrada apÃ³s mvKnob existir)
    ...
    -- SPEED/MOVE copia automaticamente o NOT STEALING do SPEED/DUEL
...
-- ============================================================
-- ESTADO GLOBAL DOS TOGGLES (salva/restaura entre sessoes)
-- ============================================================
local TOGGLES_CONFIG = "TogglesState.json"

-- Hitbox state (consolidado para economizar locais)
...
local togglesState = {
    ...
    autoDuel        = false,
    lock            = false,
    aimBatHub       = false,  -- aim bat sub-hub
    dropBrainrot    = false,  -- drop brainrot sub-hub
    fov             = false,
    meleeAimbot     = false,
    spin            = false,
    infJump         = false,
    ...
    noAnim          = false,
    playerEsp       = false,
    float           = false,  -- mostra/esconde o sub-hub Float
    autoTP          = false,  -- auto teleporta ao tomar dano/ragdoll
    hitTP           = false,  -- teleporta automaticamente ao perder o Brainrot
    autoSteal       = false,  -- auto steal
    hitboxExp       = false,  -- hitbox expander
    autoBat         = false,  -- auto bat aim
}
...
local function carregarToggles()
    ...
    elseif isfile and isfile(TOGGLES_CONFIG) then
        -- Migracao: le arquivo legado na primeira vez
    ...
    -- Estes nunca restauram ativos (efeitos visuais/perf que devem ser opt-in)
    -- lock restaura normalmente (controla visibilidade do sub-hub)
end
carregarToggles()

-- Tabela para guardar funcoes forceState de cada toggle (para restaurar estado)
local toggleForces = {}

-- ToggleSaved: cria toggle que salva estado e retorna funcao de controle externo
-- saveState=true ? salva e restaura  |  saveState=false ? nunca restaura
local function ToggleSaved(page, label, stateKey, saveState, cb)
    ...
    -- forceState: muda visual + executa cb. Guard contra loop de reentrada.
    local _busy = false
...
-- onAimbotToggled removido - sem funcao de aimbot
...
-- LOCK - mostra/esconde sub-hub. Se fechar com aimbot ativo, para o aimbot.
...
-- MELEE AIMBOT (salva) - so controla visibilidade do sub-hub
...
-- ============================================================
-- DROP BRAINROT (touch fling one-shot)
-- ============================================================
local _doDropBrainrot = nil  -- declarado antes do bloco 1
local _dbRedDot = nil
...
-- Bloco 2: logica (escopo separado para nao estourar limite de locals)
do
...
do
    ...
    resetRow.MouseButton1Click:Connect(function()
        ...
        -- Move frames visiveis imediatamente
        ...
        -- Aim: atualiza dados internos para persistir
...
-- ============================================================
-- AUTO LOAD FOR KICK (com bypass bks + estado persistente)
-- ============================================================
do
    -- Bypass: hookeia debug.info para ignorar o Synchronizer (anti-cheat)
    pcall(function()
    ...
    -- Estado persistente no getgenv: sobrevive ao reload do script
    if getgenv then
    ...
    -- Protecao anti-reload-infinito: guarda timestamp do ultimo reload
    -- Se recarregou ha menos de 5s, nao recarrega de novo
    if getgenv then
    ...
    local function doReload()
        -- Protecao anti-loop: so recarrega se passou mais de 5s desde o ultimo reload
        local now = tick()
    ...
    local function setAutoLoad(v)
        autoLoadEnabled = v
        -- Persiste no getgenv para que o proximo reload saiba que estava ON
        ...
        -- Borda vermelha quando ativo, invisivel quando inativo
        ...
        if v then
            -- Usa Players.LocalPlayer.OnTeleport para detectar kick com mais precisao
            -- Fallback: PlayerRemoving com verificacao de estado
            kickConn = Players.PlayerRemoving:Connect(function(player)
                if player ~= lp then return end
                -- Verifica se ainda esta no jogo (kick = remocao forcada)
                -- Protecao: nao recarrega se o jogador saiu voluntariamente
                -- (nao ha como distinguir 100%, mas o debounce de 5s previne loops)
                doReload()
            end);
        end
    end

    -- Restaura estado ON se estava ativo antes do reload
...
-- ============================================================
-- TOGGLES - SETTINGS (velocidades e configs dos scripts)
-- ============================================================
do
    -- Helper: cria titulo de secao dentro da aba Settings
    ...
    -- Helper: cria row de input numerico
    local function makeNumRow(labelText, getValue, setValue, minV, maxV, decimals)
        ...
        -- Atualiza valor real apos todos os modulos carregarem
        task.defer(function()
            task.wait(0.6)
            if box and box.Parent then box.Text = tostring(getValue()) end
        end);

        -- Sempre mostra o valor atual ao focar
    ...
    -- â”€â”€ DUEL SPEEDS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    -- â”€â”€ AIM SPEEDS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    -- â”€â”€ GRAB / STEAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    -- â”€â”€ KEYBINDS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    makeSettingsTitle("â–¸ KEYBINDS")

    -- Helper: cria row de keybind com label + botao [KEY]
    -- onSet(keyCode) chamado quando usuario escolhe uma tecla
    -- getKey() retorna o KeyCode atual (ou nil)
    -- getSave()/setSave(name) le/salva no config
    local function makeKbRow(labelText, getKey, onSet, getSaveKey, configSection, configKey)
        ...
        -- Restaura keybind salva do config
...
-- FLOAT (salva) - mostra/esconde sub-hub Float
...
-- OPTIMIZER (salva) - delay para nao bugar ao restaurar
...
ToggleSaved("Desync", "Desync No Reset", "desyncNoReset", true, function(v)
    -- LÃ³gica do Desync No Reset aqui
end);

ToggleSaved("Desync", "Desync Reset", "desyncReset", true, function(v)
    -- LÃ³gica do Desync Reset aqui
end);

-- TOGGLES - VISUAL
-- ============================================================

-- XRAY BASE (salva) - delay para nao bugar ao restaurar
...
local function showToast(message)
    ...
    -- mesmo gradiente do Hub principal
...
-- ============================================================
(function()
    -- Menu pos: px/py em pixels absolutos, com fallback Scale
    local menuPosDados = { px = -1, py = -1 }
    do
        local d = cfgGet("menuPos") or {px=43, py=13}
        if d then
            -- Migra formato antigo (Scale X/Y + Offset XO/YO)
    ...
    -- UIS.InputEnded global: dispara mesmo se o dedo sair do botao durante o drag
    UIS.InputEnded:Connect(function(i)
        if not pressing then return end
        ...
        if MainFrame.Visible then
            -- Fechar: encolhe pro centro e some
            ...
            -- Abrir: pop-in suave a partir do centro
            local w = 198; local h = 240
            MainFrame.BackgroundTransparency = 1
            MainFrame.Size = UDim2.new(0, w * 0.75, 0, h * 0.75)
            -- Ancora a posicao pelo canto superior esquerdo para nao pular
            local ap = MainFrame.AbsolutePosition
            ...
            -- Borda vermelha escura ao abrir o menu
...
-- Inicializa modulos (sem ativar)
pcall(function() spinModule = createSpinHub() end);
pcall(function() duelModule = createDuelHub() end);
pcall(function() createSpeedHub() end);

-- (intro removido - animacao sÃ³ no hub principal)

-- Restaura estados salvos: visual + callbacks via forceState
task.defer(function()
    task.wait(0.4)
    -- Todos os toggles que devem persistir entre sessoes
    local keys = {
        "lock", "autoDuel", "spin", "speedCustomizer",
        "meleeAimbot", "infJump", "antiRagdoll", "noAnim", "playerEsp",
        "float",  -- restaura sub-hub Float
        ...
        "hitboxExp",
        "autoBat",
        "aimBatHub",   -- restaura Aim Bat sub-hub
        "dropBrainrot", -- restaura Drop Brainrot sub-hub
    }
    -- Restaura sem animacao (instant)
    _hubRestoring = true

...
local Config = {
    StrokeThickness = 2,
    ESPColor = Color3.fromRGB(180, 0, 255), -- Updated to Purple
    BTNSColor = Color3.fromRGB(255, 0, 150), -- Updated to Hot Pink
    AnimDisable = false
}
...
local function CreateGUI()
    if CoreGui:FindFirstChild("RoxanneControlUI") then CoreGui.RoxanneControlUI:Destroy() end
    if player.PlayerGui:FindFirstChild("RoxanneControlUI") then player.PlayerGui.RoxanneControlUI:Destroy() end

    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "RoxanneControlUI"
    ...
    ButtonContainer.Size = UDim2.new(0.8, 0, 0.5, 0)
    ...
    local function createStyledButton(text)
        ...
        btnStroke.Color = Config.ESPColor
    ...
    local DesyncBtn = createStyledButton("Initialize Desync")
    DesyncBtn.Parent = ButtonContainer
    ...
    DesyncBtn.MouseButton1Click:Connect(function()
        ...
        respawn(player)
        task.wait(5.1)
        DesyncBtn.Text = "Initialize Desync"
        DesyncBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
    end)

    local AnimBtn = createStyledButton("Animations: ON")
    ...
    AnimBtn.MouseButton1Click:Connect(function()
        ...
        if Config.AnimDisable then
            ...
            AnimBtn.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    ...
    CreditFrame.ClipsDescendants = true
    CreditFrame.Position = UDim2.new(0, 0, 0.85, 0)
    CreditFrame.Size = UDim2.new(1, 0, 0, 20)

    local CreditText = Instance.new("TextLabel")
    CreditText.Parent = CreditFrame
    CreditText.BackgroundTransparency = 1
    CreditText.Position = UDim2.new(1, 0, 0, 0)
    CreditText.Size = UDim2.new(1, 0, 1, 0)
    CreditText.Font = Enum.Font.GothamMedium
    CreditText.Text = "Roxanne Utility • Network Desync Protocol Active • 2026 Edition"
    ...
    task.spawn(function()
        while true do
            CreditText.Position = UDim2.new(1, 0, 0, 0)
            local tween = TweenService:Create(CreditText, TweenInfo.new(10, Enum.EasingStyle.Linear), {Position = UDim2.new(-2, 0, 0, 0)})
            tween:Play()
